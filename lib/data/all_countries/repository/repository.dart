  ...
  Future<Either<Failure, List<CountryModal>>> fetAllCountries() async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        print("time 50= ${DateTime.now().millisecondsSinceEpoch - oldTime} ms");
        return Right(CountriesListResponse.fromJson(res.data).toDomain());
      }else{
        print("time 53= ${DateTime.now().millisecondsSinceEpoch - oldTime} ms");
        return Left(ServerFailure(message: "Server Error"));
      }
    } on SocketException catch (e) {
      print("time 57= ${DateTime.now().millisecondsSinceEpoch - oldTime} ms");
      return Left(ClientFailure(message: 'Internet connection failed')
          .orGeneric("Internet connection failed"));
    } on DioError catch (e) {
      //FirebaseCrashlytics.instance.recordError(e, StackTrace.current);
      print("time62 = ${DateTime.now().millisecondsSinceEpoch - oldTime} ms");
      return Left(ClientFailure(message: e.message)
          .orGeneric("Something went wrong. Please try again later."));
    } catch (e) {
      print("time 66= ${DateTime.now().millisecondsSinceEpoch - oldTime} ms");
      return Left(ServerFailure(message: 'Server error')
          .orGeneric("Something went wrong. Please try again later."));
    }
  }
