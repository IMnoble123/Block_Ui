...
class CustomSliderThumbCircle extends SliderComponentShape {
  ...
  void paint(
    ...
    // if (getValueInt(value) == min) {
    //   canvas.drawPath(
    //       getOuterPath(
    //         Rect.fromLTWH(0, center.dy - ((thumbRadius * 1.8) /2) , thumbRadius * 3, thumbRadius * 1.8),
    //         textDirection: TextDirection.ltr,
    //       ),
    //       paint);
    // } else {
  ...
  Path getOuterPath(Rect rect, {required TextDirection textDirection}) {
    return Path()
      ..addRRect(
          BorderRadius.circular(10).resolve(textDirection).toRRect(rect));
  }
    ...
    context.canvas.drawRRect(
      RRect.fromLTRBAndCorners(
        ...
        bottomLeft: (textDirection == TextDirection.ltr)
            ? activeTrackRadius
            : trackRadius,
        bottomRight: value >= max ? trackRadius : Radius.zero,
        topRight: value == max ? trackRadius : Radius.zero,
      ),
      leftTrackPaint,
    );
    context.canvas.drawRRect(
