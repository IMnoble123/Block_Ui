...
class PaymentPageBottomSecondView extends StatefulWidget {
  const PaymentPageBottomSecondView({Key? key, required this.currency, required this.state, required this.pageController}) : super(key: key);
 final String currency;
 final CheckoutState state;
 final PageController pageController;
  @override
  State<PaymentPageBottomSecondView> createState() => _PaymentPageBottomSecondViewState();
}

class _PaymentPageBottomSecondViewState extends State<PaymentPageBottomSecondView> {
              ...
              RadioListTile<PaymentMode>(
                ...
                title: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'Pay '
                          '${getPriceFormattedWithCODE(widget.currency, widget.state.orderTotal)} Today',
                      style: Theme.of(context)
                          .textTheme
                          .subtitle1
                          ?.copyWith(
                  ...
                  children: [
                    Text(
                      '12 Subscription Payments of '
                          '${ widget.state.couponStateModal == null ? getPriceFormattedWithCODE(widget.currency, widget.state.orderTotal / 12) : getPriceFormattedWithCODE(widget.currency, widget.state.orderTotal / 12)} per month:',
                      style: Theme.of(context)
                          .textTheme
                          .bodyText1
                          ?.copyWith(
                    ...
                    Text(
                      'Pay '
                          '${widget.state.couponStateModal == null ? getPriceFormattedWithCODE(widget.currency, (widget.state.orderTotal / 12)) : getPriceFormattedWithCODE(widget.currency, widget.state.orderTotal / 12)} Today',
                      style: Theme.of(context)
                          .textTheme
                          .subtitle1
                          ?.copyWith(
  ...
  _pay(
    ...
    Future.delayed(Duration.zero, () async {
      ...
      if (value.paymentStatus == PaymentStatus.success) {
        ...
        final createOrderRes =
        await CheckoutHelper.createOrderAndRouteToTanksPageOnSuccessPayment(
            checkoutState, billingAddressModal, value, paymentMode);
        if (!mounted) return;
        createOrderRes.fold((l) {
